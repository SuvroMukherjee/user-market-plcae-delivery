...
import { allBuyNowItems } from "../../store/buyNowSlice";
...
export const BuyProductCart = () => {
  ...
  const checkOrderDeliverity = async (userAddress, products) => {
    const data = await getDistance(
      userAddress?.pincode,
      cartItems?.proId?.sellerId?.Shop_Details_Info?.pin_code
    );

    const distanceResult = {
      productId: cartItems?.proId?._id,
      distance:
        data?.results?.[
          cartItems?.proId?.sellerId?.Shop_Details_Info?.pin_code
        ],
     ...
     return distanceResult?.isable === true;
  };
          ...
          <div className="row">
            <div className="col-lg-8 left-col">
              <div className="top-row">
                <ul className="steps">
                  <li className="completed">Bag1</li>
                  <li>Address</li>
                  <li>Payment</li>
                </ul>
              </div>
