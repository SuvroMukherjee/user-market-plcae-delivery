...
import { allBuyNowItems, tempBuyItems } from "../../store/buyNowSlice";
import {
  fetchCartLists,
...
export const TempBuyProductcart = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const cartItemsData = useSelector(allBuyNowItems);
  const tempCartItemsData = useSelector(tempBuyItems);
  const cartState = useSelector((state) => state.cart);

  const isLoggIn = useSelector((state) => state.auth.isLoggIn);

  console.log({ tempCartItemsData });
  ...
  const getPincodeDistance = async (productId, Sellercode, deliveryCode) => {
    try {
      ...
      return {
        ...
        isable: data?.results?.[deliveryCode] < 500,
      };
    } catch (error) {
  ...
  const checkOrderDeliverity = async (userAddress, products) => {
    ...
    const promises = products.map((ele) =>
      getPincodeDistance(
        ele?.proId?._id,
        ele?.proId?.sellerId?.Shop_Details_Info?.pin_code,
        userAddress?.pincode
      )
    );
  ...
  const productPlace = async () => {
    ...
    if (allAvl) {
      ...
      (await checkOrderDeliverity(deliveyAddress, cartItems?.products)) == false
    ) {
      toast.error(
          ...
          <div className="row">
            <div className="col-lg-8 left-col">
              <div className="top-row">
                <ul className="steps">
                  <li className="completed">Bag 100</li>
                  <li>Address</li>
                  <li>Payment</li>
                </ul>
              </div>
              ...
              {cartItems && (
                <div
                  className={
                    cartItems?.proId?.available_qty > 0
                      ? "cart-product-box"
                ...
                >
                  <div className="img-box">
                    <img
                      src={cartItems?.proId?.specId?.image[0]?.image_path}
                      alt="Product"
                    />
                  </div>
                  <div className="content">
                    <div className="category">
                      {cartItems?.proId?.estimateDate && (
                        <div
                          style={{
                            fontSize: "14px",
                            color: "darkgreen",
                        ...
                        >
                          ...
                          <span className="mx-2">
                            {ExpectedDeliverydata(
                              parseInt(cartItems?.proId?.estimateDate || 0),
                              cartItems?.proId?.updatedAt
                            )}
                          </span>
                        </div>
                      )}
                      <div className="d-flex flex-row w-75 justify-content-between align-items-center">
                        <div>
                          {cartItems?.proId?.productId?.categoryId?.title}
                        </div>
                      </div>
                      <span className="outStockProduct mx-2">
                        {cartItems?.proId?.available_qty < 1 && `out of stock`}{" "}
                      </span>
                    </div>
                    <div className="name">
                      {cartItems?.proId?.productId?.name}{" "}
                      {cartItems?.proId?.specId?.spec_det?.length > 0 && (
                        <span>
                          (
                          {cartItems?.proId?.specId?.spec_det
                            ?.slice(0, 3)
                            ?.map((ele, index, array) => (
                              <span key={index}>
                                {ele?.value}
                    ...
                    <div className="supplier">
                      Sold by:{" "}
                      {
                        cartItems?.proId?.sellerId?.Shop_Details_Info
                          ?.shope_name
                      }
                    </div>
                    <div className="color-qty">
                      <div className="wrap">
                        <label>Quantity:&nbsp; {cartItems?.quantity}</label>
                      </div>
                    </div>
                    <div className="price">
                      ₹{cartItems?.totalPrice?.toLocaleString()}{" "}
                      <span style={{ textDecoration: "line-through" }}>
                        ₹
                        {(
                          cartItems?.proId?.specId?.price * cartItems?.quantity
                        ).toLocaleString()}
                      </span>
            ...
            {cartItems && (
              <div className="col-lg-4 right-col mb-4">
                <label className="content-heading">
                  ...
                  {cartItems?.proId?.productId?.productId})
                </label>
                <ul className="desc-list">
                  ...
                  <li>
                    ...
                    <label>
                      + ₹{cartItems?.shipping_price?.toLocaleString()}
                    </label>
                  </li>
                </ul>
